	...
	for y in range(int(yLength)):			
		...
		for x in range(int(xLength)):
			if cellList[x][y].isHidden == False:
				print(str(cellList[x][y].status) + ' ', end = '')
			else:
				print('- ', end = '')
		print('')
	print('')
...
while(True):
	xLength = input("Enter grid length: ")
	yLength = input("Enter grid height: ")
...
while True:
	xInput = input("Enter X co-ordinate or Q to quit: ")
	...
	yInput = input("Enter Y co-ordinate or Q to quit: ")
	if xInput.upper() == 'Q': 
		print('Bye!')
		break
